...
object ConnectedComponentsExample {
  def main(args: Array[String]): Unit = {
    ...
    val graph = GraphLoader.edgeListFile(sc, "/ConnectedComponentsInput/followers.txt")
    // Find the connected components
    val cc = graph.connectedComponents().vertices
    // Join the connected components with the usernames
    val users = sc.textFile("/ConnectedComponentsInput/users.txt").map { line =>
    ...
    ccByUsername.saveAsTextFile("/ConnectedComponentsOutput")
    // $example off$
    spark.stop()
  }
}
